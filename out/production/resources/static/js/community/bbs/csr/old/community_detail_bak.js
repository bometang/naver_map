...
//        const $commentCnt = document.querySelector('.commentCnt span');
    ...
    async function listPagination() {
      try {
        ...
        // PaginationUI 최초 1회만 생성
        if (!pagination) {
          const handlePageChange = page => {
            currentPage = page;
            return getPostCommentList(page, recordsPerPage);
          };
        ...
        pagination.setTotalRecords(total);                 // 총 건수 갱신

        await getPostCommentList(currentPage, recordsPerPage); // 현재 페이지 유지
      } catch (err) {
        console.error(err);
      }
    }
    ...
    //댓글 버튼
        async function EndPagination(){
          ...
          try {
            ...
            console.log($commentCnt.textContent);
            $commentCnt.textContent = `댓글 ${totalRecords}`;
